...
function hasUsableUrl(sd) {
  const all = [...(sd.formats || []), ...(sd.adaptive_formats || [])]
  return all.some(f => f.url || f.signature_cipher || f.cipher)
}
  ...
  for (const client of CLIENTS) {
    try {
      ...
      if (sd && ((sd.formats && sd.formats.length) || (sd.adaptive_formats && sd.adaptive_formats.length))) {
        if (hasUsableUrl(sd)) return { yt, info }
        errs.push(`${client}: format ada tapi tanpa URL`)
        continue
      }
      errs.push(
        `${client}: ${(info.playability_status && info.playability_status.reason) || 'tanpa data streaming'}`
      )
    } catch (e) {
