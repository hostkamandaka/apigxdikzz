...
const VERSION = 'v7-clients'
const VALID_FORMATS = ['mp3', 'mp4', 'm4a']
const CLIENTS = ['WEB_CREATOR', 'TV_SIMPLY', 'YTMUSIC', 'YTMUSIC_ANDROID', 'YTKIDS', 'TV_EMBEDDED', 'IOS', 'ANDROID', 'WEB_EMBEDDED', 'WEB']
const UA =
...
function fail(res, code, error) {
  return res.status(code).json({ status: false, creator: CREATOR, version: VERSION, error: `[${VERSION}] ${error}` })
}
...
module.exports = function (app) {
  ...
  // ---------- Tes: cek file ini benar-benar dijalankan server ----------
  app.get('/youtube/ping', (req, res) => {
