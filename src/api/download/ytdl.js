...
const CLIENTS = ['TV_EMBEDDED', 'IOS', 'ANDROID', 'MWEB', 'TV', 'WEB_EMBEDDED', 'WEB']
const UA =
...
async function getInfo(videoId) {
  ...
  for (const client of CLIENTS) {
    try {
      ...
      lastErr = e // lanjut coba client berikutnya, tiap client bisa beda hasil
    }
  }
...
function deepLink(o, depth = 0) {
  ...
  if (typeof o === 'string') {
    return /^https?:\/\//.test(o) && !/ytimg|\.(jpe?g|png|webp)(\?|$)/i.test(o) ? o : null
  }
  if (typeof o !== 'object') return null
  const pref = ['download', 'url', 'link', 'video', 'audio', 'mp3', 'mp4', 'dl', 'downloadUrl']
  for (const k of pref) {
    if (k in o) {
